<template>
  <div class="grid-list-product-card">
    <div class="product-container">
      <product-card v-for="(product,index) in listProduct" :key="index" :hide="!(indexView <= index && index <= indexView + 5)"></product-card>
    </div>
    <div class="button-action flex flex-between" v-if="listProduct?.length > 6">
        ...
        <div class="button-next icon24 next" @click="handleClickButtonAction(1)" v-if="indexView + 5 < listProduct?.length - 1">
